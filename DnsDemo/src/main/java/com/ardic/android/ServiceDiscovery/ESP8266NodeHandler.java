...
import com.ardic.android.sampleiotigniteprojectwithdynamicconfigurations.BuildConfig;
...
public class ESP8266NodeHandler  implements ConnectionCallback,PacketListener {
    ...
    private boolean isNodeIDReceived=false;
    ...
    private  Timer sockeTimer = new Timer();
    ...
    private void initIgniteVariables(){
        ...
        if(myNode.isRegistered()){
            ...
            Log.i(TAG, myNode.getNodeID() + " Registering Things!");
    ...
    private void onPacketHandler(String packet) {
        ...
        if(packet.charAt(0) == '&'){
            ...
            isNodeIDReceived = true;
            NODE = packet.substring(1,packet.length()-1);
            Log.i(TAG, "Node ID RECEIVED!!! :  " + NODE);
            SharedPreferences.Editor editor = applicatonContext.getSharedPreferences("NODE_PREFS",Context.MODE_PRIVATE).edit();
            ...
            if (igniteConnected) {
                ...
                isRunning = true;
                initIgniteVariables();
                if (myReaderThread != null && igniteConnected) {
    ...
    private void getNodeIDFromPrefs(){
        SharedPreferences prefs = applicatonContext.getSharedPreferences("NODE_PREFS", Context.MODE_PRIVATE);
        String restoredText = prefs.getString("NODE", null);

        Log.i(TAG,"NODE Recovered 1: " + NODE);
        if (restoredText != null) {
            String node = prefs.getString("NODE",null);//"No name defined" is the default value.
            Log.i(TAG,"NODE Recovered  2: " + NODE);
            if(node!=null){
                NODE = node;
                Log.i(TAG,"NODE Recovered 3 : " + NODE);
            }
        }
    }
}
